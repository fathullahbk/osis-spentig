...
export default function ProfileList({ profiles, deleteProfile, updateProfile }: any) {
  const [isDeleting, setIsDeleting] = useState<number | null>(null);

  const handleDelete = async (id: number) => {
    setIsDeleting(id);
    
    // Menggunakan toast.promise agar user tahu proses sedang berjalan
            ...
            {profiles.map((profile: any) => (
              <div key={profile.id} className="p-4 border border-slate-100 rounded-lg bg-white flex items-center gap-4 group hover:border-blue-200 transition-all shadow-sm hover:shadow-md">
                ...
                <div className="flex gap-1">
                  {/* Tombol Edit (Bisa kamu sambungkan ke fungsi edit nanti) */}
                  <button
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Apakah Anda yakin? Tindakan ini akan menghapus data <strong>{profile.name}</strong> secara permanen dari server.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
